...
function xmldb_videotimeplugin_repository_upgrade($oldversion) {
    ...
    if ($oldversion < 2021060200) {
        ...
        // Rename field user on table videotime_vimeo_video to userinfo.
        $table = new xmldb_table('videotime_vimeo_video');
        $field = new xmldb_field('user', XMLDB_TYPE_TEXT, null, null, null, null, null, 'uri');

        // Launch rename field userinfo.
        $dbman->rename_field($table, $field, 'userinfo');

        // Repository savepoint reached.
        upgrade_plugin_savepoint(true, 2021060200, 'videotimeplugin', 'repository');
    }
