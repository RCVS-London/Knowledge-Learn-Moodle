...
$authenticationstatus = get_string('done', 'videotime');
$authenticationclass = null;
$configurationstatus = get_string('configure_vimeo_help', 'videotime', [
...
$settingsstatus = get_string('done', 'videotime');
...
try {
    ...
    if ($accesstoken = get_config('videotime', 'vimeo_access_token')) {
        ...
        if ($response['status'] == 200) {
            $authenticationstatus = get_string('authenticated', 'videotime');
            $authenticationclass = 'success';
        } else {
            $authenticationstatus = get_string('needs_authentication', 'videotime') . ' (' . $response['status'] . ')';
            $authenticationclass = 'danger';
        }
    }
} catch (api_not_authenticated $e) {
    $authenticationstatus = get_string('todo', 'videotime');
} catch (api_not_configured $e) {
    $settingsstatus = get_string('todo', 'videotime');
    $authenticationstatus = get_string('todo', 'videotime');
}
